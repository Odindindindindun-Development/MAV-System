...
const BillingDetail: React.FC = () => {
  ...
  const handleRemoveAdjustment = async (adjId: number) => {
    ...
    try {
      ...
      setAdjustments(prev =>
        prev.filter(a => a.BillingAdjustmentID !== adjId)
      );
  ...
  const handleAddPayment = async () => {
    ...
    if (selectedDate < minDate) {
      return alert(
        "Payment date is too old. Please enter a realistic date."
      );
    }
